...
int main() {
    ...
    if (tasklet_id == 15) {
        mram_base_addr_bias = (uint32_t) (DPU_MRAM_HEAP_POINTER + mram_offset + communicate_Y_pad_byte + 2*n_size_pad_W_fp_byte);
        ...
        while (current_batch_id < nr_batches) { 
            ...
            for (uint32_t batch_size_fraction_id = 0; batch_size_fraction_id < b_size_frac; ++batch_size_fraction_id) {
                ...
                if (tasklet_id == 0) { 
                    ...
                    label = cache_Y[batch_size_fraction_id] == 1 ? one : minus_one;
                    if (label == minus_one) {
            ...
            for (uint32_t i = 0; i < samples_loop_transfer; ++i) {
                ...
                barrier_wait(&my_barrier);
                result->cycles_mram_to_wram += timer_stop(&cycles);
                result->bytes_mram_to_wram += (uint64_t) (3*transfer);
