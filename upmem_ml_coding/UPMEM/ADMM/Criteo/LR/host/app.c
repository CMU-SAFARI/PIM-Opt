...
void* compute_Z_ADMM_global_parallel_function(void* thread_info_list){
    ...
    if (thread_id != 0 && thread_id != 62) {
        // Computing Z_ADMM_global: soft_threshholding_operation
        for (uint64_t x = 0; x < number_of_features_per_thread; ++x) {
...
int main(int argc, char **argv) {
    ...
    uint32_t array_reg_term_alpha[] = {6}; // reg_term_alpha in ADMM jargon corresponds to alpha
                ...
                for(uint32_t ee = 0; ee < array_reg_term_length; ++ee) { 
                    ...
                    for (uint32_t se = 0; se < array_reg_term_alpha_length; ++se) { 
                        ...
                        for (uint32_t le = 0; le < array_task_epochs_length; ++le) {
                            ...
                            if (strong_scaling == 0) {
                                sprintf(filename_experiment_dpu, "%s/results_UPMEM/Criteo/weak_scaling/admm_LR_uint32/lr_reg_DPU_arch__criteo__uint32__ADMM__NR_DPUS_%u__NR_TASKLETS_%u__m_size_%lu__m_test_size_%lu__n_size_%lu__b_size_frac_%u__nr_batches_%u__learning_rate_%u__reg_term_%u__alpha_%u__epochs_%u__task_epochs_%u.txt", DEST_DIR, NR_DPUS, NR_TASKLETS, m_size, number_of_test_samples, n_size, b_size_frac, nr_batches, learning_rate, reg_term, reg_term_alpha, epochs, task_epochs);
                            } else {
                                sprintf(filename_experiment_dpu, "%s/results_UPMEM/Criteo/strong_scaling/admm_LR_uint32/lr_reg_DPU_arch__criteo__uint32__ADMM__NR_DPUS_%u__NR_TASKLETS_%u__m_size_%lu__m_test_size_%lu__n_size_%lu__b_size_frac_%u__nr_batches_%u__learning_rate_%u__reg_term_%u__alpha_%u__epochs_%u__task_epochs_%u.txt", DEST_DIR, NR_DPUS, NR_TASKLETS, m_size, number_of_test_samples, n_size, b_size_frac, nr_batches, learning_rate, reg_term, reg_term_alpha, epochs, task_epochs);
                            }
                            ...
                            for (uint64_t s = 0; s < 63; ++s) {
                                ...
                                Z_ADMM_thread_info_compute_parallel_list[s].soft_threshholding_parameter = soft_threshholding_parameter;
                            ...
                            for (uint64_t i = 0; i < epochs; ++i) { 
                                ...
                                *cross_entropy_loss += l1_loss;
                                ...
                                printf("Epoch %lu. Training cross entropy loss of averaged model = %.9f, only cross entropy loss = %.9f, only l1 loss = %.9f\n", i, *cross_entropy_loss, only_cross_entropy_loss, l1_loss);
                                ...
                                *cross_entropy_loss += l1_loss;
                                ...
                                printf("Epoch %lu. Test cross entropy loss of averaged model = %.9f, only cross entropy loss = %.9f, only l1 loss = %.9f\n\n", i, *cross_entropy_loss, only_cross_entropy_loss, l1_loss);
                            }
