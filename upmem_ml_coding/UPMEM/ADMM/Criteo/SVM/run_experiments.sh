...
for j in "${NR_TASKLETS[@]}"; do
    for i in "${NR_DPUS[@]}"; do
        for s in "${SCALING[@]}"; do
            ...
            log_file="${model}__${alg}__Criteo__NR_DPUS_${i}_NR_TASKLETS_${j}_SCALING_${s}.log"
            start_time=$(date +%s)
            ...
            echo "${model}__${alg}__Criteo__NR_DPUS_${i}_NR_TASKLETS_${j}_SCALING_${s}, runtime: $hours h $minutes min $seconds s" | tee -a "$top_log_file"
