...
void* gradient_average_function(void* thread_info_list){
    ...
    LS* W_local_gradients_buffer = thread_info_list_current->W_local_gradients_buffer;
    ...
    for (uint64_t m = 0; m < NR_DPUS; ++m) {
        for (uint64_t x = 0; x < number_of_features_per_thread; ++x) {
            W_global_gradient_buffer[x] += W_local_gradients_buffer[m*n_size + x];
        }
    }
    if (thread_id != 0 && thread_id != 62) {
        for (uint64_t x = 0; x < number_of_features_per_thread; ++x) {
            LS check_tmp = (LS) (W_global_gradient_buffer[x] >> b_size_frac_log);
            LS check_tmp_reg = (LS) (((LS) (W_host_float[x])) >> learning_rate_plus_reg_term);
            ...
            LS tmp = (LS) (((LS) (W_host_float[x])) - gradient_update); 
        ...
        for (uint64_t x = 0; x < number_of_features_per_thread; ++x) {
            ...
            } else if (x == bound_start_set_zero) { // update bias term
                LS check_tmp = (LS) (W_global_gradient_buffer[x] >> b_size_frac_log);
                LS gradient_update = (LS) (0);
                if (check_tmp != -1) {
                    gradient_update = check_tmp;
...
int main(int argc, char **argv) {
    ...
    uint32_t array_b_size_frac[] = {131072,262144,524288,1048576};
    ...
    LS* W_local_gradients_buffer = (LS*) calloc(((uint64_t) (NR_DPUS))*n_size, sizeof(LS));
            ...
            for (uint32_t ke = 0; ke < array_epochs_length; ++ke) {
                ...
                for(uint32_t ee = 0; ee < array_reg_term_length; ++ee) { 
                    ...
                    for (uint32_t le = 0; le < array_task_epochs_length; ++le) {
                        ...
                        memset(W_local_gradients_buffer, 0, ((uint64_t) (NR_DPUS))*n_size*sizeof(LS));
                        ...
                        if (strong_scaling == 0) {
                            sprintf(filename_experiment_dpu, "%s/results_UPMEM/Criteo/weak_scaling/ga_SVM_uint32/svm_reg_DPU_arch__criteo__uint32__GA__NR_DPUS_%u__NR_TASKLETS_%u__m_size_%lu__m_test_size_%lu__n_size_%lu__b_size_frac_%u__nr_batches_%u__learning_rate_%u__reg_term_%u__epochs_%u__task_epochs_%u.txt", DEST_DIR, NR_DPUS, NR_TASKLETS, m_size, number_of_test_samples, n_size, b_size_frac, nr_batches, learning_rate, reg_term, epochs, task_epochs);
                        } else {
                            sprintf(filename_experiment_dpu, "%s/results_UPMEM/Criteo/strong_scaling/ga_SVM_uint32/svm_reg_DPU_arch__criteo__uint32__GA__NR_DPUS_%u__NR_TASKLETS_%u__m_size_%lu__m_test_size_%lu__n_size_%lu__b_size_frac_%u__nr_batches_%u__learning_rate_%u__reg_term_%u__epochs_%u__task_epochs_%u.txt", DEST_DIR, NR_DPUS, NR_TASKLETS, m_size, number_of_test_samples, n_size, b_size_frac, nr_batches, learning_rate, reg_term, epochs, task_epochs);
                        }
                        ...
                        gradient_average_parallel_t* thread_info_gradient_average_parallel_list = (gradient_average_parallel_t*) calloc(63, sizeof(gradient_average_parallel_t));
                        ...
                        for (uint64_t s = 0; s < 63; ++s) {
                            ...
                            thread_info_gradient_average_parallel_list[s].W_host_float = W_dpu_fp + s * start_index_thread_model_average;
                            ...
                            thread_info_gradient_average_parallel_list[s].number_of_features_per_thread = (uint64_t) (15896);// number of features of model
                            ...
                            thread_info_gradient_average_parallel_list[s].bound_start_set_zero = (uint64_t) (0);
                            if (s == 62) {
                                thread_info_gradient_average_parallel_list[s].number_of_features_per_thread = (uint64_t) (15920);
                                thread_info_gradient_average_parallel_list[s].bound_start_set_zero = (uint64_t) (14448);
                            }
                        }
                        ...
                        for(uint64_t rep = 0; rep < epochs; ++rep) { 
                            ...
                            for (uint32_t current_batch_id = 0; current_batch_id < nr_batches; ++current_batch_id) { 
                                ...
                                i = 0;
                                DPU_FOREACH(dpu_set, dpu, i) {
                                    DPU_ASSERT(dpu_prepare_xfer(dpu, W_local_gradients_buffer + ((uint64_t) (i)) * n_size_pad_W_fp)); 
                                }
                                DPU_ASSERT(dpu_push_xfer(dpu_set, DPU_XFER_FROM_DPU, "DPU_RESULTS", 0, n_size_pad_W_fp * sizeof(LS), \
                                DPU_XFER_DEFAULT));
                                ...
                                for (uint32_t s = 0; s < 63; ++s) {
                                    int32_t thread_return_value = pthread_create(&model_average_worker_thread[s], NULL, gradient_average_function, (void *) &thread_info_gradient_average_parallel_list[s]);
                        ...
                        for (uint64_t i = 0; i < epochs; ++i) { 
                            ...
                            // Compute training hinge loss
                            fprintf(fp_dpu, "Epoch %lu. Compute training hinge loss.\n", i);
                            printf("Epoch %lu. Compute training hinge loss.\n", i);
                            ...
                            // l2_loss = sqrt(l2_loss); --> we have squared so no sqrt necessary
                            double reg_term_double = (double) pow(2, reg_term+1); // reg_term * 0.5 s.t. gradient is nicer
                            ...
                            *hinge_loss += l2_loss;
                            fprintf(fp_dpu, "Epoch %lu. Training hinge loss of averaged model = %.9f, only hinge loss = %.9f, only l2 loss = %.9f\n", i, *hinge_loss, only_hinge_loss, l2_loss);
                            printf("Epoch %lu. Training hinge loss of averaged model = %.9f, only hinge loss = %.9f, only l2 loss = %.9f\n", i, *hinge_loss, only_hinge_loss, l2_loss);
                            // Compute test hinge loss
                            memset(hinge_loss, 0, sizeof(double));
                            memset(compute_loss_worker_list, 0, 64*sizeof(double));
                            fprintf(fp_dpu, "Epoch %lu. Compute test hinge loss.\n", i);
                            printf("Epoch %lu. Compute test hinge loss.\n", i);
                            for (uint64_t s = 0; s < 64; ++s) {
                                thread_info_compute_loss_list[s].hinge_loss = compute_loss_worker_list + s;
                                thread_info_compute_loss_list[s].n_size = (uint64_t) n_size;
                                thread_info_compute_loss_list[s].n_size_samples = (uint64_t) n_size_samples;
                            ...
                            *hinge_loss += l2_loss;
                            ...
                            printf("Epoch %lu. Test hinge loss of averaged model = %.9f, only hinge loss = %.9f, only l2 loss = %.9f\n\n", i, *hinge_loss, only_hinge_loss, l2_loss);
                        }
