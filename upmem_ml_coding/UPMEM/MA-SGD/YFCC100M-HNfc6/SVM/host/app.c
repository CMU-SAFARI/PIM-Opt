...
void* model_average_function(void* thread_info_list){
    ...
    if (thread_id < 16) { 
        ...
        LS* bias_local_models_buffer = thread_info_list_current->bias_local_models_buffer;
        LS* bias_global_model_buffer = thread_info_list_current->bias_global_model_buffer;
        for (uint64_t m = 0; m < NR_DPUS; ++m) {
            *bias_global_model_buffer += bias_local_models_buffer[m];
        }
        LS tmp = *bias_global_model_buffer / NR_DPUS;
        if (tmp > overflow_test || tmp < underflow_test) {
            if (tmp > overflow_test) {
                *bias_global_model_buffer = (LS) (overflow_test - 1);
            } else {
                *bias_global_model_buffer = (LS) (underflow_test + 1);
            }
        } else {
            *bias_global_model_buffer = tmp;
        }
...
int main(int argc, char **argv) {
    ...
    S* W_local_models_buffer = (S*) calloc(NR_DPUS*n_size, sizeof(S));
    LS* W_global_model_buffer = (LS*) calloc(n_size, sizeof(LS));
    LS* bias_local_models_buffer = (LS*) calloc(NR_DPUS, sizeof(LS));
    LS* bias_global_model_buffer = (LS*) calloc(1, sizeof(LS));
            ...
            for (uint32_t ke = 0; ke < array_epochs_length; ++ke) {
                ...
                for(uint32_t ee = 0; ee < array_reg_term_length; ++ee) { 
                    ...
                    for (uint32_t le = 0; le < array_task_epochs_length; ++le) {
                        ...
                        memset(W_local_models_buffer, 0, NR_DPUS*n_size*sizeof(S));
                        memset(bias_local_models_buffer, 0, NR_DPUS*sizeof(LS));
                        memset(bias_global_model_buffer, 0, 1*sizeof(LS));
                        ...
                        if (strong_scaling == 0) {
                            sprintf(filename_experiment_dpu, "%s/results_UPMEM/YFCC100M-HNfc6/weak_scaling/mbsgd_SVM_uint32/svm_reg_DPU_arch__yfcc100m__uint32__MB_SGD_per_DPU__NR_DPUS_%u__NR_TASKLETS_%u__m_size_%lu__m_test_size_%lu__n_size_%u__b_size_frac_%u__nr_batches_%u__learning_rate_%u__reg_term_%u__epochs_%u__task_epochs_%u.txt", DEST_DIR, NR_DPUS, NR_TASKLETS, m_size, number_of_test_samples, n_size, b_size_frac, nr_batches, learning_rate, reg_term, epochs, task_epochs);
                        } else {
                            sprintf(filename_experiment_dpu, "%s/results_UPMEM/YFCC100M-HNfc6/strong_scaling/mbsgd_SVM_uint32/svm_reg_DPU_arch__yfcc100m__uint32__MB_SGD_per_DPU__NR_DPUS_%u__NR_TASKLETS_%u__m_size_%lu__m_test_size_%lu__n_size_%u__b_size_frac_%u__nr_batches_%u__learning_rate_%u__reg_term_%u__epochs_%u__task_epochs_%u.txt", DEST_DIR, NR_DPUS, NR_TASKLETS, m_size, number_of_test_samples, n_size, b_size_frac, nr_batches, learning_rate, reg_term, epochs, task_epochs);
                        }
                        ...
                        for (uint64_t s = 0; s < 17; ++s) {
                            ...
                            if (s < 16) {
                                ...
                                thread_info_model_average_parallel_list[s].W_dpu_fp = W_dpu_fp + s * start_index_thread_model_average;
                                ...
                                thread_info_model_average_parallel_list[s].bias_local_models_buffer = bias_local_models_buffer;
                                thread_info_model_average_parallel_list[s].bias_global_model_buffer = bias_global_model_buffer;
                            }
                        ...
                        for(uint64_t rep = 0; rep < epochs; ++rep) { 
                            ...
                            for (uint32_t current_batch_id = 0; current_batch_id < nr_batches; ++current_batch_id) { 
                                ...
                                i = 0; 
                                DPU_FOREACH(dpu_set, dpu, i) {
                                    DPU_ASSERT(dpu_prepare_xfer(dpu, bias_global_model_buffer)); 
                                }
                                DPU_ASSERT(dpu_push_xfer(dpu_set, DPU_XFER_TO_DPU, DPU_MRAM_HEAP_POINTER_NAME, \
                                ...
                                i = 0;
                                DPU_FOREACH(dpu_set, dpu, i) {
                                    DPU_ASSERT(dpu_prepare_xfer(dpu, W_local_models_buffer + i * n_size_pad_W_fp)); 
                                }
                                DPU_ASSERT(dpu_push_xfer(dpu_set, DPU_XFER_FROM_DPU, DPU_MRAM_HEAP_POINTER_NAME, max_rows_per_dpu * n_size_pad * sizeof(U) + communicate_Y_pad * sizeof(U), n_size_pad_W_fp * sizeof(S), \
                                    DPU_XFER_DEFAULT)); 
                                i = 0;
                                DPU_FOREACH(dpu_set, dpu, i) {
                                    DPU_ASSERT(dpu_prepare_xfer(dpu, bias_local_models_buffer + i)); 
                                }
                                ...
                                clock_gettime(CLOCK_MONOTONIC, &start);
                                memset(W_global_model_buffer, 0, n_size*sizeof(LS));
                                memset(bias_global_model_buffer, 0, sizeof(LS));
                            ...
                            *bias_dpu_float_error = (float) *bias_global_model_buffer / CAST;
                        ...
                        for (uint64_t i = 0; i < epochs; ++i) { 
                            ...
                            for (uint64_t s = 0; s < 64; ++s) {
                                ...
                                thread_info_compute_error_rate_list[s].bias_global_model_buffer = bias_checkpoint_for_compute_error + i;
                                thread_info_compute_loss_list[s].bias_global_model_buffer = bias_checkpoint_for_compute_error + i;
                            }
                            memset(error_rate, 0, sizeof(float));
                            memset(reduction, 0, sizeof(uint64_t));
                            memset(sum_of_Y, 0, sizeof(uint64_t));
    ...
    free(bias_local_models_buffer);
    free(bias_global_model_buffer);
