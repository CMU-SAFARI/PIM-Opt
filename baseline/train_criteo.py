  ...
  "criteo" : {
    "cpu" : {
      ...
      "svm" : {
        ...
        "ga-sgd" : {
          "batch_size" : [1024, 2048, 4096, 8192],    # *128
          "learning_rate": [2],
          "weight_decay": [12],
          "alpha": [-1]
        },
...
def main():
  ...
  for num_global_epochs in [10]:
    for num_local_epochs in [1]:
      ...
      if train_args.optim != "admm":
        if train_args.device != "gpu":
          optim = train_args.dist_type + "-" + optim
      for batch_size in train_params[train_args.dataset][train_args.device][train_args.model_type][optim]["batch_size"]:
        for lr in train_params[train_args.dataset][train_args.device][train_args.model_type][optim]["learning_rate"]:
          for wd in train_params[train_args.dataset][train_args.device][train_args.model_type][optim]["weight_decay"]:
            for alpha in train_params[train_args.dataset][train_args.device][train_args.model_type][optim]["alpha"]:
              for num_procs in [128]:
                ...
                train_args.num_procs = num_procs
